...
Text::Text(){
    ...
    _fontInfo.firstASCII = 35;
    _fontInfo.lastASCII = 290;
...
void Text::scrollText(const char* _pInput)
{
    static uint8_t charWidth = 0;
    static uint8_t cBuf[8];  // this should be ok for all built-in fonts
    static uint8_t i = 0;
    ...
    case 0: 
        ...
        charWidth = getChar(*pMsg++, sizeof(cBuf) / sizeof(cBuf[0]), cBuf);
        i = 0;
        textState = 1;
    case 1: 
        Transform(TSL);
